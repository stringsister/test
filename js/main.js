...
fetch(apiUrl)
    ...
    .then(data => {
        ...
        // Create HTML for all 5-star reviews
        const reviewsHTML = fiveStarReviews.map(review => `
        ...
        // Insert all 5-star reviews into the 'google-reviews' div
...
function initializeScroll() {
    ...
    function scrollReviews(direction) {
        const currentReview = direction === 1 ? 1 : 0;
        const translateValue = -currentReview * (300 + 10); // 300 is the width of each review container, 10 is the margin
        reviewsContainer.style.transform = `translateX(${translateValue}px)`;
    }

    document.querySelector('.scroll-btn:first-child').addEventListener('click', () => scrollReviews(-1));
    document.querySelector('.scroll-btn:last-child').addEventListener('click', () => scrollReviews(1));
}
